        ...
        while not self.stopped:
            ...
            while not self.stopped:
                ...
                if not ret:
                    ...
                    if consecutive_failures >= 2: # 响应策略：连续2次失败即判定断线
                        log_msg("监控故障", "视频流帧读取连续失败，判定已断线！正在触发自愈重连程序...")
                        self.connected = False
                        try:
        ...
        if candidate_count >= STABILIZATION_FRAMES:
            ...
            if gesture_detected != last_published_gesture:
                try:
                    ...
                    mqtt_client.publish(MQTT_TOPIC, mqtt_gesture, retain=True)
                    log_msg("系统识别", f"成功感应有效手势: '{gesture_detected}'，已推送纯净状态: '{mqtt_gesture}'")
                except Exception as e:
